...
import { getPrebuiltIslandTerrainPieces } from './prebuiltIsland';
...
export class FloatingIslandScene extends Phaser.Scene {
  ...
  private islandPatches: GrassPatch[] = [];
  ...
  preload() {
    ...
    this.load.image(SECOND_LAYER_SHADOW_KEY, tinySwordsAssets.shadow);
  ...
  private buildScene() {
    ...
    this.islandPatches = cloneGrassPatches(seaLevelScenePlan.initialIslandPatches);
    this.grassPatches = cloneGrassPatches(seaLevelScenePlan.initialGrassPatches);
  ...
  private placeTerrainPatch(tool: TerrainTool, anchor: GridCell) {
    if (tool.layer === 'base') {
      const nextPatches = placeGrassPatch({
        ...
        blockedCells: this.getOccupiedIslandCells(),
        availableCells: this.availableOverlayCells,
      });
    ...
    const nextPatches = placeSecondLayerPatch({
      ...
      baseCells: this.getOccupiedBaseCells(),
    });
  ...
  private renderTerrainPatches() {
    if (
      !this.waterFoamRoot
      || !this.grassRoot
      || !this.secondLayerShadowRoot
      || !this.secondLayerRoot
      || !this.occupiedCellRoot
    ) return;

    this.waterFoamRoot.removeAll(true);
    this.grassRoot.removeAll(true);
    this.secondLayerShadowRoot.removeAll(true);
    this.secondLayerRoot.removeAll(true);
    this.occupiedCellRoot.removeAll(true);

    const gridLeft = -placementWidth / 2;
    const gridTop = -placementHeight / 2;
    const islandCells = this.getOccupiedIslandCells();
    ...
    for (const piece of getPrebuiltIslandTerrainPieces({ occupiedCells: islandCells })) {
  ...
  private createTerrainPiece(
    ...
    alpha: number,
    offsetY: number = 0,
    displayHeight: number = TILE_SIZE,
  ) {
    ...
    tile.setDisplaySize(TILE_SIZE + 1, displayHeight + 1);
    tile.setAlpha(alpha);
    return tile;
  }

  private createSecondLayerShadow(
    piece: TerrainShadowPiece,
    gridLeft: number,
    gridTop: number,
  ) {
  ...
  private getBaseAvailableCells() {
    return getGrassMapCells({
      grid: seaLevelScenePlan.grid,
      occupiedCells: this.getOccupiedBaseCells(),
  ...
  private getAvailableOverlayCells() {
    if (this.getSelectedTerrainTool()?.layer === 'second') {
      return this.getOccupiedBaseCells();
    }
  ...
  private getOccupiedIslandCells() {
    return this.islandPatches.flatMap((patch) => patch.cells);
  }

  private getOccupiedBaseCells() {
    return [...this.getOccupiedIslandCells(), ...this.getOccupiedGrassCells()];
  }
...
function cloneGrassPatches(patches: readonly GrassPatch[]) {
